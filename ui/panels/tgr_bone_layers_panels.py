...
class TGR_PT_View3D_Panel_BoneLayers(TGR_PT_BASE):
    ...
    def __init__(self) -> None:
        ...
        if len(self.layer_collection) == 0:
            ...
            self.layer_collection[2].name = "Mechanism Layer"
    ...
    def draw(self, context):
        ...
        for i, layer in enumerate(tgr_layers):
            ...
            row.operator('tgr.lock_bones_from_layer', icon=lock_icon, text="",
                         depress=layer.lock_selection).layer_name = layer.name
            row.separator()
